...
def mean_shift_with_seeds(X, Z, kappa, max_iters=10, metric='cosine', embedding_alpha=0.02):
    """ Runs mean-shift
    ...
    cluster_labels = connected_components(Z, 2 * embedding_alpha, metric=metric)  # Set epsilon = 0.1 = 2*alpha
...
def _vis_minibatch_segmentation_final(image, label, out_label=None, out_label_refined=None,
...
def test_sample(sample, network, network_crop, visualize=True):
    ...
    if network_crop is not None:
        rgb_crop, out_label_crop, rois = crop_rois(image, out_label.clone(), depth)
    ...
    if visualize:
        bbox = None
        _vis_minibatch_segmentation_final(image, label, out_label, out_label_refined, features,
